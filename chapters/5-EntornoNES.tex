...
integrado para la creación de videojuegos \delODR{de}\newODR{para} la consola
\textit{NES}.
...
Este entorno de desarrollo consistirá \delODR{de}\newODR{en} un editor
...
\textit{NES4JAMS} añade \newODR{a \emph{JAMS}} un nuevo tipo de proyecto
que incorpora todas las herramientas para el desarrollo
de videojuegos \delODR{de}\newODR{para} \textit{NES}.
...
principal muy similar a la de los proyectos \textit{MIPS32}. \note{Y
  por comentarios como este creo que hay que dedicar algo más de
  espacio a introducir JAMS en el estado del arte, porque el lector no
  tiene ni idea de cómo es la ventana principal de JAMS en los
  proyectos MIPS32}


\section{Editor}\label{sec:editor}

Los desarrolladores de videojuegos \delODR{de}\newODR{para} \textit{NES} trabajan con
dos tipos de archivo\newODR{s} principales: los archivos \textit{asm}
...
\textit{NES4JAMS} \delODR{implementa}\newODR{incorpora/añade} dos editores nuevos a \textit{JAMS}:
...
cuatro valores diferentes.\note{Óscar: ¿cuáles? No puedes presuponer
  que el lector conoce sobradamente el formato PCX. Quizá deberías
  explicar ciertos detalles de la NES, como cuántos colores tenía, etc}
...
derecha del editor.
Por último, el editor \delODR{da}\newODR{ofrece} la opción de \textbf{guardar una copia}
...
\note{Óscar: En este párrafo hay mucho ``ensamblador''. Hay que redactarlo
  nuevamente ;)}
...
$print-1$.\note{Óscar: me he perdido con el símbolo ``*''}
...
Estas equivalencias relacionan una etiqueta \delODR{a}\newODR{con} una expresión matemática
que a la vez puede \delODR{usar}\newODR{hacer uso de} otras etiquetas para calcular su valor.
...
\note{Óscar: yo no haría subsubsections, porque son epígrafes muy
  cortos. ¿Qué tal como ítems?}
...
la división \delODR{o las operaciones}\newODR{u operaciones lógicas} a nivel de bit.
...
ser un entorno de desarrollo \delODR{para}\newODR{de} videojuegos \delODR{de}\newODR{para} la \textit{NES}
...
\delODR{El simulador para la consola \textit{NES} no presenta un visualizador
de instrucciones como nodo principal, si no}\newODR{El nodo principal del simulador para la consola \emph{NES} no
...
Este comportamiento, junt\delODR{ad}o con otras características como
...
de veces por segundo.\note{Óscar: ¿Qué quieres decir en este párrafo?
 ¿Qué significa ``una manera óptima de modificar imágenes de manera
  constante''? Etc.}
...
de la salida de vídeo.\note{Óscar: ¿por qué ``imágenes por segundo''?
  ¿Qué quieres decir?}
...
Esta memoria es llamada \textit{OAM} \delODR{o}\newODR{u} \textit{Object Attribute Memory},
