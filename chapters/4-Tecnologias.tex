...
Al tener todos los elementos \delODR{de} correctamente etiquetados, \textit{JAMS}
...
que \delODR{los elementos escuchando pueden recibir}\newODR{podrán
  recibir los elementos que estén escuchando}.
...
\delODR{pide}\newODR{¿invoca a?} un elemento de tipo
...
Si una escucha \delODR{pide el}\newODR{¿invoca al?} evento $Event$, su método se ejecutará siempre que un evento ocurra.
...
Esta última opción permite \delODR{a los objetos
  eliminarse}\newODR{que los objetos sean eliminados} de la memoria
...
de las escuchas evita\newODR{n} que la aplicación mantenga elementos de un
...
sistema de tareas basado en \textbf{ejecutadores}\note{Óscar: yo diría
  que en cristiano es ejecutores, o eso dice la RAE}.
...
\ref{fig:jams-assembling}.\note{Remaquetar esta figura. No puedes
  dejarla solita en la siguiente página}
